...
let findGrapeArrow = (arr) => {for(let i=0; i<arr.length; i++) {if(arr[i].color === "purple") {console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`)}}}
...
let printParams = (param1, param2) => console.log(`The first parameter is ${param1}. The second parameter is ${param2}.`)
...
invoke the callback, passing in a concatenation of firstName and lastName. Then, outside of the greeting function, invoke the greeting function,
passing in the value firstName and lastName of your choice, and a callback function that takes a parameter of fullName.
...
greeting("Alex", "Sutton", callback)

function greeting(firstName, lastName, callback) {
    return callback(firstName + " " + lastName);
}

greeting('John', 'Doe', function(fullName) {
    console.log('Hello, my full name is ' + fullName);
});
